...
for(const required of ['<title>BURNED','COMING SOON','EMBER','Burn Zone','average entry','3%','15 minutes','id="mechanics"','id="rules"']) if(!html.includes(required)) throw new Error('Missing site content: '+required);
for(const match of html.matchAll(/href="#([^"]+)"/g)) if(!html.includes('id="'+match[1]+'"')) throw new Error('Broken anchor: '+match[1]);
for(const name of ['style.css','burned-logo.png','favicon.png']) if(!(await stat(join('public',name))).size) throw new Error('Empty asset: '+name);
