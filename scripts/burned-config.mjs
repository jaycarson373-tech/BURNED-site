...
export function burnedPublicConfig(env = {}) {
  ...
  const dexscreenerUrl = indexVerified && mint ? `https://dexscreener.com/solana/${mint}` : '';
  return { supabaseUrl: indexVerified ? supabaseUrl : '', supabaseKey: indexVerified ? supabaseKey : '', projectId, xUrl, topblastMint: indexVerified ? mint : '', rewardMint: indexVerified ? rewardMint : '', dexscreenerUrl, indexVerified, rewardsActive: indexVerified && read('BURNED_REWARDS_ACTIVE') === 'true' };
}
