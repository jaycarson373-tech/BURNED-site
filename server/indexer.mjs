...
async function rebuild(finalizedBlock){
  ...
  const rewardEvents=persisted.rewards.map(row=>({id:row.id,type:'reward',wallet:row.wallet,label:'QQQ RECEIVED',amountLabel:`${q(row.amountRaw,launch.pairDecimals).toLocaleString(undefined,{maximumFractionDigits:4})} QQQ`,timestamp:new Date(row.timestamp*1000).toISOString(),transactionHash:row.transactionHash}));
  const tradeEvents=trades.map(row=>({id:row.id,type:row.type,wallet:row.wallet,label:row.type==='buy'?'TOPBLAST BUY':'SELL',amountLabel:`${row.quoteAmount.toLocaleString(undefined,{maximumFractionDigits:4})} QQQ`,timestamp:new Date(row.timestamp*1000).toISOString(),transactionHash:row.transactionHash}));
